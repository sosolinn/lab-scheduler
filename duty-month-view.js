(() => {
  let __labVisibleDutyMonth = new Date();
  __labVisibleDutyMonth = new Date(
    __labVisibleDutyMonth.getFullYear(),
    __labVisibleDutyMonth.getMonth(),
    1
  );

  function __labDutyMonthStart(date) {
    return new Date(date.getFullYear(), date.getMonth(), 1);
  }

  function __labAddDutyMonths(date, amount) {
    return new Date(date.getFullYear(), date.getMonth() + amount, 1);
  }

  function __labDutyMonthKey(date) {
    return `${date.getFullYear()}-${padNumber(date.getMonth() + 1)}`;
  }

  function __labFormatDutyMonth(date) {
    return `${date.getFullYear()}年${date.getMonth() + 1}月`;
  }

  function __labEnsureDutyMonthNavigation() {
    ...
    if (!navigation) {
      ...
      navigation
        ...
        ?.addEventListener("click", () => {
          __labVisibleDutyMonth = __labAddDutyMonths(
            __labVisibleDutyMonth,
            -1
          );
          renderDuties();
        });
      ...
      navigation
        ...
        ?.addEventListener("click", () => {
          __labVisibleDutyMonth = __labDutyMonthStart(new Date());
          renderDuties();
        });
  ...
  function __labRenderMonthlyDutyRecord(duty) {
  ...
  renderDuties = function renderDutiesByMonth() {
    const summary = __labEnsureDutyMonthNavigation();
    const monthKey = __labDutyMonthKey(__labVisibleDutyMonth);
    const monthDuties = sortDuties(duties).filter((duty) =>
      String(duty.date || "").startsWith(monthKey)
    );

    if (summary) {
      summary.textContent = `${__labFormatDutyMonth(__labVisibleDutyMonth)} · ${monthDuties.length} 条记录`;
    }
        ...
        return `
          <section class="duty-month-day-group${isToday ? " today" : ""}">
            ...
            <div class="duty-month-day-records">
              ${records.map(__labRenderMonthlyDutyRecord).join("")}
            </div>
          </section>
        `;
      })
  ...
  dutyDateInput?.addEventListener("change", () => {
    ...
    __labVisibleDutyMonth = __labDutyMonthStart(
  ...
  __labEnsureDutyMonthNavigation();
  renderDuties();
})();
